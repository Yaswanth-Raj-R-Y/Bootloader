...
void uart_send_msg_app(char*);      // Send a string over USART1
...
int main()
{
    ...
    // Enable clock for GPIOC (for LED output)
    RCC->AHB1ENR |= RCC_AHB1ENR_GPIOCEN;

    // Configure PC13 as output (clear bits, then set as output)
    GPIOC->MODER &= ~(3 << 26);     // Clear mode bits for PC13
    GPIOC->MODER |= (1 << 26);      // Set PC13 as general purpose output

    while (1)
    {
        GPIOC->ODR ^= (1 << 13);    // Toggle PC13 output (LED blink)
