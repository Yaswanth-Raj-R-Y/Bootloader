...
int main()
{
    ...
    // Set PA9 and PA10 to alternate function mode for USART1 TX/RX
    ...
    // Configure PB4 as output (clear bits, then set as output)
    GPIOB->MODER &= ~(3 << 8);    // Clear mode bits for PB4
    GPIOB->MODER |= (1 << 8);     // Set PB4 as general purpose output

    while (1)
    {
        GPIOB->ODR ^= (1 << 4);   // Toggle PB4 output (LED blink)
